...
public class MainActivity extends AppCompatActivity {
    ...
    class ProgressTask extends AsyncTask<Integer,Integer,Integer> {
        ...
        protected Integer doInBackground(Integer... params) {//UI접근하면 안됨
            for (int i = 0; i < 10 && isRunning; i++) {
                ...
                value+=10; publishProgress(value);//onProgressUpdate 호출
            }
            return null;
        }
        @Override
        ...
        @Override
        protected void onPostExecute(Integer integer) {//백그라운드 작업후에 호출
    ...
    public class ProgressRunnable implements Runnable {//스레드대신 메인UI 접근
        @Override
        public void run() {
    ...
    protected void onStart() {
        super.onStart();
        progressBar1.setProgress(0); progressBar2.setProgress(0);//프로그레스초기화
        ...
        Thread thread2 = new Thread(new Runnable() {//두번째 스레드 (0.2초에 한번)
            @Override
            public void run() {
                try {
